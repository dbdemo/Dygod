...
import db.com.dyhome.bean.UrlMapEntity;
...
public class HaveDataFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
    ...
    public HaveDataFragment() {
    }

    public HaveDataFragment(UrlMapEntity urlMapEntity) {
        this.urlMapEntity = urlMapEntity;
    }
...
//        urlMapEntity = args.getParcelable("urlMapEntity");
        System.out.println(this + " ===:=== " + urlMapEntity.getUrlData().get("url"));
        if (JapanView != null) {
            return JapanView;
        }
    ...
    private void initCache() {
        ...
        mainDataServant.getNewsData(urlMapEntity.getUrlData(), false, true, currentPageIndex, mNetWorkListener);
    ...
    private RecommendNewsRecyAdapter.RecyclerViewItemClickListener mItemClickListener = new RecommendNewsRecyAdapter.RecyclerViewItemClickListener() {
        ...
        public void onItemClick(View view, int position) {
            if (mDialog == null) {
                mDialog = new StyleDialog(HaveDataFragment.this.getContext(), "正在获取数据");
            }
            mDialog.show();
            //根据地址获取电影信息
            ...
            movieInfoServant.getMovieInfoData(UrlConstant.mainUrl + data.get(position).getTitlinkle(), new NetWorkListener<MovieInfoEntity>() {
                ...
                public void successful(MovieInfoEntity movieInfoEntity) {
                    mDialog.dismiss();
                    MovieInfoActivity.start(HaveDataFragment.this.getContext(), movieInfoEntity);
                }
